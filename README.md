...
```bash
Template$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Template$ sam local start-api
Template$ curl http://localhost:3000/
```
...
```bash
Template$ sam logs -n HelloWorldFunction --stack-name Template --tail
```
